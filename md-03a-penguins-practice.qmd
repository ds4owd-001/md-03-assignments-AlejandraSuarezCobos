...
5.  What data types are contained in the variables? (Reminder: <https://ds4owd-001.github.io/website/slides/lec-02-visualisation.html#/types-of-variables>)
    ...
    categorical variables, continuous variables, discrete variables
...
```{r}
penguins |> 
  filter(species=="Adelie",bill_length_mm<35,sex=="female")
```
...
```{r}
penguins |> 
  mutate(boddy_mass_kg=body_mass_g/1000)
...
```{r}
penguins |> 
  group_by(species)|>summarise(flip_max=max(flipper_length_mm,na.rm =TRUE),
...
```{r}
penguins |> 
  filter(sex=="female",island%in% c("Torgersen","Biscoe")) |> count(island,sex)
```
